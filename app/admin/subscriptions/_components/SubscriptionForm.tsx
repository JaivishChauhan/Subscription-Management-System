...
export function SubscriptionForm({
  ...
  const defaultValues = useMemo<SubscriptionFormInput>(
    () => ({
      ...
      expirationDate: formatDateInput(
        initialSubscription?.expirationDate ?? null
      ),
      notes: initialSubscription?.notes ?? "",
      lines: initialSubscription?.lines.map((line) => ({
  ...
  const taxTotal = watchedLines.reduce(
  ...
  async function onSubmit(values: SubscriptionCreateInput) {
    ...
    try {
      ...
      const nextId =
        mode === "edit" ? initialSubscription?.id : result.subscription?.id
      router.push(
        nextId
          ? `${basePath}/subscriptions/${nextId}`
  ...
  function applyProductDefaults(
    ...
    const nextUnitPrice = roundCurrency(
      (planPrice ?? product.salesPrice) + variantExtraPrice
    )
  ...
  function maybeRecalculateTax(
    ...
    setValue(`lines.${index}.taxAmount`, resolvedTaxAmount, {
      shouldDirty: true,
    })
  }
  ...
  return (
    <div className="space-y-6">
        ...
        {initialSubscription ? (
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold">
              {initialSubscription.subscriptionNumber}
            </span>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-900">
                This flow needs customers, plans, and products before a
      ...
      <form
        ...
        className="grid gap-6 lg:grid-cols-[1.4fr_0.95fr]"
      >
        <section className="border-border bg-card space-y-5 rounded-3xl border p-6 shadow-sm">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-sm">
              Draft and quotation subscriptions stay editable. Confirmed and
              active records become operationally read-only.
            </p>
          </div>
          ...
          <div className="border-border bg-muted/15 space-y-4 rounded-3xl border p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold">Order Lines</h2>
                <p className="text-muted-foreground text-sm">
                  Products, quantity, optional tax rule, and stored tax amount
                  per line.
                </p>
              </div>
              {editable ? (
                <button
                  ...
                  className="border-border hover:bg-background inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition-colors"
                >
                ...
                return (
                  ...
                  >
                    <div className="grid gap-4 lg:grid-cols-[1.25fr_1fr_0.7fr_0.8fr_0.9fr_0.9fr_auto]">
                      ...
                      <Field
                        label="Variant"
                        error={lineErrors?.variantId?.message}
                      >
                        <select
                          disabled={
                            !editable ||
                      ...
                      <Field
                        label="Tax Rule"
                        error={lineErrors?.taxId?.message}
                      >
                        <select
                          disabled={!editable}
                      ...
                      <Field
                        label="Tax Amount"
                        error={lineErrors?.taxAmount?.message}
                      >
                        <input
                          type="number"
                          min="0"
          ...
          <section className="border-border bg-card rounded-3xl border p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Live totals based on the current draft.
                </p>
              </div>
            </div>
            ...
            {selectedPlan ? (
              <div className="border-border bg-muted/20 mt-5 rounded-2xl border p-4 text-sm">
                <p className="font-semibold">
                  Plan minimum quantity: {selectedPlan.minQuantity}
                </p>
          ...
          <section className="border-border bg-card rounded-3xl border p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Built against the current schema.
                </p>
              </div>
            </div>
            ...
            <ul className="text-muted-foreground mt-4 space-y-3 text-sm">
              <li>
                Draft and quotation subscriptions can be edited; later states
                are read-only.
              </li>
              <li>
                Tax rules can auto-calculate line tax amounts, but the schema
                stores tax amounts rather than tax-rule relations.
              </li>
              <li>
                Activation is implemented as a status transition now; invoice
                generation lands next.
              </li>
            </ul>
          </section>
        </aside>
