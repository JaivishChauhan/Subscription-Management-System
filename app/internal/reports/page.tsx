...
export default async function InternalReportsPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: totalPaymentsAgg._count.id,
      icon: IconCreditCard,
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="border-border bg-card rounded-2xl border p-5">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            From {paidInvoicesAgg._count.id} paid invoices
          </p>
        </div>
        <div className="border-border bg-card rounded-2xl border p-5">
          <p className="text-muted-foreground text-xs font-semibold tracking-widest uppercase">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            From {pendingInvoicesAgg._count.id} confirmed invoices
          </p>
        </div>
        <div className="border-border bg-card rounded-2xl border p-5">
          <p className="text-muted-foreground text-xs font-semibold tracking-widest uppercase">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {totalPaymentsAgg._count.id} payments recorded
          </p>
        </div>
      </div>
      ...
      <div className="border-border bg-card rounded-2xl border p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {(
            ["draft", "quotation", "confirmed", "active", "closed"] as const
          ).map((status) => (
            <div
              key={status}
              className="border-border bg-muted/40 rounded-xl border p-4 text-center"
            >
              <p className="text-2xl font-bold">{statusMap[status] ?? 0}</p>
              <p className="text-muted-foreground mt-1 text-xs font-semibold capitalize">
                {status}
              </p>
            </div>
          ))}
        ...
        <div className="space-y-3">
          {recentPayments.length === 0 ? (
            ...
            recentPayments.map((p) => (
              <div
                key={p.id}
                className="bg-muted/30 flex items-center justify-between rounded-xl px-4 py-3"
              >
                <div>
