...
export const subscriptionCreateSchema = subscriptionBaseSchema.superRefine((data, ctx) => {
...
export const subscriptionUpdateSchema = subscriptionBaseSchema.superRefine((data, ctx) => {
  if (data.startDate && data.expirationDate && data.expirationDate < data.startDate) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["expirationDate"],
