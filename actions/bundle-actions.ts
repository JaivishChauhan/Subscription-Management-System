...
export async function createBundleAction(data: BundleFormData) {
  ...
  const bundle = await prisma.bundle.create({
    data: {
      ...
      services: {
        create: data.serviceIds.map((serviceId, order) => ({ serviceId, order })),
      },
    },
 ...
 * @security Admin-only.
 */
export async function updateBundleAction(id: string, data: Partial<BundleFormData>) {
  await requireAdmin();
 ...
 * @security Admin-only.
 */
export async function toggleBundleFeaturedAction(id: string, isFeatured: boolean) {
  await requireAdmin();
